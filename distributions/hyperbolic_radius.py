...
from distributions.ars import ARS
...
class HyperbolicRadius(dist.Distribution):
    ...
    def log_prob(self, value):
        res = - value.pow(2) / (2 * self.scale.pow(2)) + (self.dim - 1) * logsinh(self.c.sqrt() * value) - (self.dim - 1) / 2 * self.c.log() - self.log_normalizer.expand(value.shape)
