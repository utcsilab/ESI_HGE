...
def random_shapes2(image_shape,
                   max_shapes,
                   min_shapes=1,
                   min_size=2,
                   max_size=None,
    ...
    random2 = np.random.RandomState(random_seed + 1)
    random3 = np.random.RandomState(random_seed + 2)
    user_shape = shape
    ...
    if input_data is None:
        ...
        image = input_data[0].copy()
        labels = input_data[1][:]
        filled = input_data[2].copy()
        colors = input_data[3].copy()
        # image, labels, filled, colors = input_data

    colors2 = _generate_random_colors(num_shapes, num_channels, intensity_range, random2)
    if colors is None:
        colors = colors2.copy()
    else:
        # print('asdasdasd')
        # print(colors.shape)
        # print(colors2.shape)
        # print(colors.shape, colors2.shape)
        colors = np.concatenate((colors, colors2))
    total_shapes = len(colors)
    for shape_idx in range(total_shapes - num_shapes, total_shapes):
        if user_shape is None:
            shape_generator = random3.choice(SHAPE_CHOICES)
        else:
        ...
        for _ in range(num_trials):
            ...
            if allow_overlap or not filled[indices].any():
                # print(shape_idx, colors, indices)
    ...
    return image, labels, filled, colors